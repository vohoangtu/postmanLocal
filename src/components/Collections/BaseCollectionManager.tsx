/**
 * Base Collection Manager Component
 * Component cơ bản chứa tất cả logic chung cho việc quản lý collections
 * Được sử dụng bởi cả PublicCollectionManager và WorkspaceCollectionManager
 */
...
import { Folder, Share2, MessageSquare, History, FileCode, Upload, Play, Plus } from "lucide-react";

export interface BaseCollectionManagerProps {
  // Props để customize behavior
  enableCollaboration?: boolean; // Cho phép hiển thị collaboration features
  renderCollaborationFeatures?: (collectionId: string) => React.ReactNode; // Render collaboration features
  renderCollectionActions?: (collectionId: string) => React.ReactNode; // Custom collection actions
  showWorkspaceSelector?: boolean; // Hiển thị workspace selector
  filterByWorkspace?: string | null; // Filter collections theo workspace ID
  onCollectionSelected?: (collectionId: string) => void; // Callback khi collection được chọn
}

export default function BaseCollectionManager({
  enableCollaboration = false,
  renderCollaborationFeatures,
  renderCollectionActions,
  showWorkspaceSelector = true,
  filterByWorkspace,
  onCollectionSelected
}: BaseCollectionManagerProps) {
  const {
    collections,
    setCollections,
    selectedCollection,
    ...
    try {
      ...
      if (isAuthenticated) {
        ...
        if (!token) {
          ...
          // Filter collections theo workspace nếu có active workspace hoặc filterByWorkspace
          const workspaceId = filterByWorkspace || activeWorkspace;
          const url = workspaceId 
            ? `${API_BASE_URL}/collections?workspace_id=${workspaceId}`
  ...
  }, [setCollections, isAuthenticated, API_BASE_URL, filterByWorkspace, activeWorkspace]);
        ...
        try {
          const response = await fetch(`${API_BASE_URL}/collections`, {
            ...
            body: JSON.stringify({
              ...
              data: null,
              workspace_id: selectedWorkspaceId || filterByWorkspace || activeWorkspace || null,
            }),
          });
          if (response.ok) {
  ...
  // Filter collections theo active workspace hoặc filterByWorkspace
  const filteredCollections = useMemo(() => {
    ...
    return collections.filter(c => c.workspace_id === workspaceId);
  }, [collections, activeWorkspace, filterByWorkspace]);

  // Render toolbar với Environment selector và New Collection button
  const renderToolbar = () => (
    <PageToolbar
      leftSection={
        <>
          {showWorkspaceSelector && workspaces.length > 0 && (
            <Select
              label="Workspace"
              value={filterByWorkspace || activeWorkspace || ""}
              onChange={(e) => {
  ...
  // Render collection detail panel khi selected
  const renderCollectionDetail = (collection: any) => {
    if (selectedCollection !== collection.id) return null;
    ...
    return (
      <div className="mt-4 pt-4 border-t border-gray-200 dark:border-gray-700 space-y-4">
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          {renderCollectionActions && renderCollectionActions(collection.id)}
        </div>
        {enableCollaboration && showComments && (
          <div className="pt-4 border-t border-gray-200 dark:border-gray-700">
            <CommentsPanel collectionId={collection.id} />
          </div>
        )}
        {enableCollaboration && showVersionHistory && (
          <div className="pt-4 border-t border-gray-200 dark:border-gray-700">
            <VersionHistory
        ...
        {showDocumentation && (
          <div className="pt-4 border-t border-gray-200 dark:border-gray-700">
            <PublicCollectionDocumentation collectionId={collection.id} />
          </div>
        )}
        {showTestRunner && (
          <div className="pt-4 border-t border-gray-200 dark:border-gray-700">
  ...
  return (
    ...
    >
      {filteredCollections.length === 0 ? (
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {filteredCollections.map((collection) => (
              <div key={collection.id} className="space-y-0">
                ...
                />
                {/* Collaboration Features - chỉ render nếu enableCollaboration = true */}
                {enableCollaboration && renderCollaborationFeatures && (
                  <div className="mt-2">
                    {renderCollaborationFeatures(collection.id)}
                  </div>
                )}
              </div>
            ))}
          ...
          {/* Collection Detail Panel - hiển thị bên dưới grid */}
          {selectedCollection && (
            <div className="mt-6">
              {filteredCollections
                ...
                .map((collection) => (
                  <div
                    key={collection.id}
                    className="bg-white dark:bg-gray-800 border-2 border-blue-500 dark:border-blue-400 rounded-lg p-6 shadow-md"
                  >
    ...
    {/* Modals - render bên ngoài PageLayout vì chúng là fixed overlays */}
    {showCreateModal && (
            ...
            <div className="space-y-3">
              ...
              {showWorkspaceSelector && workspaces.length > 0 && (
                <Select
                  ...
                  value={selectedWorkspaceId || filterByWorkspace || activeWorkspace || ""}
