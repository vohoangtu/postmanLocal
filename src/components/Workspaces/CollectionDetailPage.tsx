...
export default function CollectionDetailPage() {
  ...
  const activeTab = useMemo(
    () => location.pathname.includes('/documentation') ? 'documentation' : 'requests',
    [location.pathname]
  );
  const requestCount = useMemo(() => collection.requests?.length || 0, [collection.requests]);
  ...
  }, [collection, activeTab, requestCount, workspaceId, collectionId, navigate, permissions.canEdit]);

  return (
    <>
      <PageLayout toolbar={renderToolbar()} padding="none">
        <div className="p-6">
          <Outlet />
        </div>
      </PageLayout>
